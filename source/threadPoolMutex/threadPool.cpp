...
void ThreadPool::runInThread()
{
    ...
    {
        //fprintf(stderr, "exception caught in ThreadPool : thread %ld\n", std::this_thread::get_id()._M_thread);
        fprintf(stderr, "reason: %s\n", ex.what());
        abort();
    }
    catch (...)
    {
        //fprintf(stderr, "unknown exception caught in ThreadPool : thread %ld\n", std::this_thread::get_id());
        throw; // rethrow
    }
